    ...
    newRows.forEach(row => {
      extentKeys.forEach(key => {
        ...
        if (fieldHasMovingAverage[key]) {
          ...
          if (
            maValue != undefined &&
            (extent[1] === null || maValue > extent[1])
          ) {
            extent[1] = maValue;
          }
        }
...
function renderMap(data, options) {
  ...
  function fillColor(d) {
    const datum = d[field];
    return datum != undefined && datum !== 0
      ? colorScale(datum)
      : mapNoDataColor;
  }
  ...
  function onMouseEnter(d) {
    ...
    if (d.label) {
      showMapTooltip({
        ...
        field,
        evt,
        allowDrilldown: allowDrilldown && !isTestingData,
      });
    }
  }
...
function renderMapLegend(clusters, min) {
  ...
  $legend
    ...
    .each(function (d, i) {
      $(this).css(
        'background-color',
        i === 0 ? mapNoDataColor : mapColors[i - 1]
      );
    })
    ...
    dataPoints = [
      {
        key: 'positive',
        color: 'primary1',
        pct: value.positivePct,
      },
      {
        key: 'negative',
        color: 'primary2',
        pct: value.negativePct,
      },
    ...
    dataPoints = [
      {
        key: 'newPositive',
        color: 'primary1',
        pct: value.newPositivePct,
      },
      {
        key: 'newNegative',
        color: 'primary2',
        pct: value.newNegativePct,
      },
      { key: 'newTests' },
      { key: 'newCases' },
      { key: 'newDeaths' },
...
function attachEvents() {
  ...
  $('#cb-consistent-y').change(function () {
    const value = $(this).is(':checked');
    router.push({
      [filterKeys.consistentY]: value ? '1' : '0',
    });
  });
...
export default function fetchAndRenderStates() {
  startLoading();
  fetchStateData()
    .then(() => {
      renderAllStates();
